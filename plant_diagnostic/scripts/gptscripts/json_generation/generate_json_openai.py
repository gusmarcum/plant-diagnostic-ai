#!/usr/bin/env python3
"""
Fail-proof MiniGPT-4 stage-2 JSONL generator (7 classes, always writes something).

- One image -> one record, guaranteed.
- Label = parent folder (drought|frost|healthy|overwatered|root_rot|gray_mold|white_mold).
- If API returns JSON {"report": "<text>"}, we use it; otherwise we write a fallback doctor-grade report.
- No brittle gates on length/structure beyond strict JSON parsing.
- Resumable: skips images already present in the output file.
- Temperature only sent for models that support it (o3 family typically ignored / not required).

Usage example:
  python scripts/minigpttrainingscripts/json_generation/generate_json_openai.py \
    --img-root data/train_aug \
    --out datasets/strawberry_stage2_train_7class.jsonl \
    --model o3 \
    --sleep 0.6
"""
...
VALID_LABELS = {"drought","frost","healthy","overwatered","root_rot","gray_mold","white_mold"}
...
def build_system_prompt() -> str:
    return (
        "You are a strawberry plant diagnostician.\n"
        "\n"
        "Rules:\n"
...
def build_user_payload(label: str) -> str:
    return json.dumps({
        "label": label,
        "note": "Use the fixed label; visual-only; one broad, detailed report."
    }, ensure_ascii=False)
...
def fallback_report(label: str) -> str:
    if label == "drought":
        return ("Diagnosis: drought. Leaves show dry, curled margins with a dull surface consistent with moisture stress. "
                "Deep-soak to the root zone and avoid frequent shallow sips; add 5–7 cm mulch while keeping it off crowns. "
    ...
    if label == "gray_mold":
        return ("Diagnosis: gray_mold. Gray, powdery sporulation on blossoms or fruit with soft collapse suggests Botrytis under cool, humid conditions. "
                "Sanitize blighted blossoms/fruit; increase airflow so leaves do not touch when wet; irrigate at the base and finish before sunrise. "
                "Next 7 days: check 10 clusters daily for new soft spots or spore dust and remove promptly.")
    if label == "white_mold":
        return ("Diagnosis: white_mold. Cottony white mycelium at stems/crown with bleached, softened tissue near soil contact indicates Sclerotinia in humid, stagnant zones. "
                "Prune affected tissue, improve spacing, and keep mulch slightly back from the crown to reduce surface wetness overnight. "
                "Next 7 days: inspect crown bases for fresh mycelium; maintain morning ventilation.")
    # healthy
    return ("Diagnosis: healthy. Foliage is evenly colored with intact margins and fruit set appropriate for the stage. "
...
def main():
    ap = argparse.ArgumentParser("Always-write generator for MiniGPT-4 stage-2 (7 classes)")
    ...
    ap.add_argument("--log-file", type=Path, default=Path("logs/generate_stage2_reports_7class.log"))
    args = ap.parse_args()
    ...
    with args.out.open("a", encoding="utf-8") as out:
        for img, label in iter_images(args.img_root):
            ...
            if not t.startswith(prefix):
                report = f"{prefix}. " + t.split("Diagnosis:", 1)[-1].strip()
            ...
            written += 1
            logging.info("[%s] %s -> 1 report", ("FALLBACK" if used_fallback else "OK"), img.relative_to(args.img_root))
            time.sleep(args.sleep)

    logging.info("DONE. Wrote %d entries -> %s (fallbacks used: %d)", written, args.out, fb_count)
