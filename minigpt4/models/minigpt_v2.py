...
class MiniGPTv2(MiniGPTBase):
    ...
    def from_config(cls, cfg):
        # Extract image size from config, with fallback to default
        img_size = cfg.get("image_size", 448)
        if not img_size:
            # Try to get from vis_processor config
            img_size = (cfg.get("vis_processor", {})
                        .get("train", {})
                        .get("image_size", 448))
        logging.info(f"Image size extracted from config: {img_size}")
        ...
        logging.info(f"Creating model with image_size={img_size}, vit_model={vit_model}")
