...
class MiniGPTBase(BaseModel):
    ...
    def __init__(
        ...
        if freeze_vit:
            ...
            self.ln_vision.eval()
            # Disable training mode changes
            self.visual_encoder.train = lambda mode=True: self.visual_encoder
            self.ln_vision.train = lambda mode=True: self.ln_vision
            logging.info("Vision encoder frozen and set to eval mode")
        else:
    ...
    def generate(
        ...
        for output_token in outputs:
            ...
            output_texts = output_texts.split(r'')[0].strip()
